...
## HTTPS (сертификат от регистратора/хостинга)
...
```powershell
New-Item -ItemType Directory -Force -Path .\certs\ваш-домен.ru | Out-Null
Copy-Item .\certificate.key .\certs\ваш-домен.ru\privkey.pem -Force
Copy-Item .\certificate.crt .\certs\ваш-домен.ru\cert.pem -Force
Copy-Item .\certificate_ca.crt .\certs\ваш-домен.ru\chain.pem -Force
Get-Content .\certs\ваш-домен.ru\cert.pem,.\certs\ваш-домен.ru\chain.pem | Set-Content -Encoding ascii .\certs\ваш-домен.ru\fullchain.pem
```

### 3) Включите HTTPS nginx
```bash
